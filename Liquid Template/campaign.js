(function () {
  ...
  if (
    location.pathname.indexOf("donors") === -1 &&
    location.pathname.indexOf("donate") === -1
  ) {
    initLandingPage();
    templateTwoSupport();
  ...
  function initLandingPage() {
    ...
    function checkCampaignStatus() {
      ...
      var originalGoalTextElem = document.getElementsByClassName(
        "original-goal-text"
      );
      var progressRing = document.getElementsByClassName("progress-ring");
      var progressFillBonusElem = document.getElementsByClassName(
        "progress-fill-bonus"
      );
      var progressPercentBonusElem = document.getElementsByClassName(
        "progress-percent-bonus"
      );
      var progressGoalBonusElem = document.getElementsByClassName(
        "progress-goal-bonus"
      );
      var progressPercentOriginal = document.getElementsByClassName(
        "progress-percent-original"
      );
      ...
      var donorTotalsContainerCheck = document.getElementsByClassName(
        "donor-totals-container-check"
      );
      var donorTotalsContainer = document.getElementsByClassName(
        "donor-totals-container mobile-hidden"
      );
      var donorListContainer = document.getElementsByClassName(
      ...
      var campaignStarted =
        clock.remaining().totalHours < CAMPAIGN_DURATION ? true : false;
      ...
      if (!campaignStarted) {
        ...
        if (HAD_BONUS_ROUND) {
          for (var i = 0; i < progressRing.length; i++) {
            ...
            progressGoalBonusElem[i].innerHTML =
              "$" + numberWithCommas(EXTENDED_GOAL - CAMPAIGN_GOAL);
            progressPercentBonusElem[i].innerHTML =
              Math.floor(
                ((TOTAL_AMOUNT_RAISED - CAMPAIGN_GOAL) /
                  (EXTENDED_GOAL - CAMPAIGN_GOAL)) *
                  100
              ) + "%";
            progressFillBonusElem[i].style.width =
              Math.floor(
                ((TOTAL_AMOUNT_RAISED - CAMPAIGN_GOAL) /
                  (EXTENDED_GOAL - CAMPAIGN_GOAL)) *
                  100
          ...
          for (var i = 0; i < progressRing.length; i++) {
            progressRing[i].style.display = "none";
            progressRaised[i].style.display = "";
            progressPercentOriginal[i].innerHTML =
              Math.floor((TOTAL_AMOUNT_RAISED / CAMPAIGN_GOAL) * 100) + "%";
          }
        }
  ...
  function initDonorsPage() {
    ...
    function checkCampaignStatus() {
      ...
      var headerImageContainer = document.getElementsByClassName(
      ...
      if (!campaignStarted) {
        ...
        headerImageContainer.innerHTML =
          headerImageContainer.innerHTML +
          "<br><br>Donations given before the campaign starts are hidden. Your donation will display when the campaign launches! Come back once we've started and check our progress!";
      } else {
        donorsRingElem.style.display = "none";
  ...
  function initDonatePage() {
    ...
    var subHeaderImage = document.getElementsByClassName(
